...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //3) check if is everything ok and send the token
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError(
        'There was an error while sending you reset email, Try agian later'
      ),
      500
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  //2) if token has not expired, make the new password
  if (!user) {
  ...
  user.passwordResetExpire = undefined;
  await user.save();
  //3) update  changededPasswordAt property for the user
